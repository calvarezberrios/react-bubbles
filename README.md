...
- [x] Explain what a token is used for.
      ...
      A token is used to authenticate a user requesting or attempting to change data on a server/api.

- [x] What steps can you take in your web apps to keep your data secure?

      Setup an authentication path using a token, protect the sensitive pages by checking if a token is available prior to loading it.

        - Axios call using Authorization header
        - PrivateRoute that redirects to a login page or other page if authentication fails, ie: no token found

- [ ] Describe how web servers work.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

  .post(), .get(), .put(), .delete() (PGPD acronym)
